...
P1_sum  = zeros(Float64, L, Nt)
P1_sum2 = zeros(Float64, L, Nt)
...
for tr in 1:Ntraj
    ...
    for k in 1:Nt
        disp[k] = average_displacement(view(P1, :, k), j0)
    end
    disp_sum  .+= disp
    disp_sum2 .+= disp.^2
...
# means
P1_avg = P1_sum ./ Ntraj
center_decay = center_sum ./ Ntraj
avg_disp = disp_sum ./ Ntraj

# standard errors (SEM)
P1_var = (P1_sum2 ./ Ntraj) .- P1_avg.^2
P1_err = sqrt.(max.(P1_var, 0.0) ./ Ntraj)

center_var = (center_sum2 ./ Ntraj) .- center_decay.^2
center_err = sqrt.(max.(center_var, 0.0) ./ Ntraj)

disp_var = (disp_sum2 ./ Ntraj) .- avg_disp.^2
disp_err = sqrt.(max.(disp_var, 0.0) ./ Ntraj)

println("pTWA ($(sampling)) Center decay P1(j0,t_final) = ", center_decay[end], " ± ", center_err[end])
println("pTWA ($(sampling)) Avg displacement <|j-j0|>(t_final) = ", avg_disp[end], " ± ", disp_err[end])

outfile = "pTWA_Z3_L$(L)_alpha$(α)_g$(g)_single_excitation_$(sampling)_N$(Ntraj).jld2"
@save outfile L α J g j0 times Ntraj seed sampling P1_avg P1_err center_decay center_err avg_disp disp_err
