...
# # ------------------------- Effective fields h_j = ∂H/∂x_j --------------------

# """
#     compute_fields!(h, x, params)

# Fill `h[j]` (3×3) with the classical "field" matrices h_j = ∂H_W/∂x_j.

# For the long-range Z-coupling (open chain):
#   H_Z = -∑_{r} J_r ∑_{j=1}^{L-r} ( Z_j Z_{j+r}* + Z_j* Z_{j+r} )
# Only depends on diagonal x_j^{aa} through Z_j.

# For the transverse field:
#   H_X = -∑_j ( f X_j + f* X_j* )
# with X_j = x^{10}+x^{21}+x^{02} (cyclic).
# This contributes constant off-diagonal entries to h_j.
# """
# function compute_fields!(h::Vector{Matrix{ComplexF64}},
#                          x::Vector{Matrix{ComplexF64}},
#                          params::PTWAParams)
...
#     # Precompute Z_j and conj(Z_j)
#     Z = Vector{ComplexF64}(undef, L)
#     Zc = Vector{ComplexF64}(undef, L)
#     for j in 1:L
#         Z[j] = Z_symbol(params, x[j])
#         Zc[j] = conj(Z[j])
#     end

#     # Interaction contribution: diagonal fields only
#     # ∂/∂x_j^{aa} [ -(Z_j Z_k* + Z_j* Z_k) ] = -(ω^a Z_k* + ω^{-a} Z_k)
#     # where ω^{-a} = conj(ω^a) because |ω|=1.
#     for r in 1:(L-1)
#         J = Jr[r]
#         if J == 0.0
#             continue
...
#             for a in 1:3
#                 ωa = ωvec[a]
#                 h[j][a,a] += -J * (ωa * Zc[k] + conj(ωa) * Z[k])
#             end

#             # Add to site k (symmetric)
#             for a in 1:3
#                 ωa = ωvec[a]
#                 h[k][a,a] += -J * (ωa * Zc[j] + conj(ωa) * Z[j])
#             end
#         end
#     end

#     # Transverse field contribution: off-diagonal cyclic entries
#     # H_X = -∑_j ( f X_j + f* X_j* )
#     # X_j depends linearly on x_j[2,1], x_j[3,2], x_j[1,3]
#     # So ∂H/∂x_j[2,1] = -f,   ∂H/∂x_j[1,2] = -f* (from X*),
#     # and similarly for the other cyclic pairs.
#     for j in 1:L
#         hj = h[j]
#         hj[2,1] += -f
#         hj[3,2] += -f
...
# """
#     rhs!(dx, x, params, work_h)

# Compute the time derivative dx for the full chain state x.

# Dynamics:
#   ẋ_j = i [ x_j, h_j ].

# `work_h` is a preallocated vector of 3×3 matrices used to store h_j.
# """
# function rhs!(dx::Vector{Matrix{ComplexF64}},
#               x::Vector{Matrix{ComplexF64}},
#               params::PTWAParams,
#               work_h::Vector{Matrix{ComplexF64}})

#     compute_fields!(work_h, x, params)

#     for j in 1:params.L
#         xj = x[j]
#         Gj = work_h[j]          # Gj = ∂H/∂x
#         hj = transpose(Gj)      # hj = (∂H/∂x)^T   <-- critical
